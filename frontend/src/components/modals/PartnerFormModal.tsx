...
export function PartnerFormModal({ partner, onClose, onSuccess }: PartnerFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await partnerService.update(partner.id, updateData);
      } else {
        // Create
      ...
      setError(err.response?.data?.message || 'Erro ao salvar parceiro');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {formData.isActive
                      ? 'Parceiro ativo e operacional'
                      : 'Parceiro desativado (pode ser reativado)'}
                  </p>
                </div>
                <button
                  type="button"
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            >
              {loading ? 'Salvando...' : isEditing ? 'Salvar Alterações' : 'Criar Parceiro'}
            </button>
          </div>
        </form>
      </div>
